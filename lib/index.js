...
class Snapshot {
    constructor(a, b) {
        if (b === null || b === undefined) {
            this.ref = a.ref;
            this.data = convertor_1.convertToOutput(a.data());
        }
        else {
            this.ref = a;
            this.data = b;
    ...
    save() {
        this.setCreatedDate();
        return this.ref.create(convertor_1.convertToInput(this.data));
    }
    saveWithBatch(batch) {
        this.setCreatedDate();
        batch.create(this.ref, convertor_1.convertToInput(this.data));
    }
    saveReferenceCollection(collectionName, snapshot) {
        const rc = this.ref.collection(collectionName).doc(snapshot.ref.id);
        return rc.create(convertor_1.convertToInput({ createdAt: new Date(), updatedAt: new Date() }));
    }
    saveReferenceCollectionWithBatch(batch, collectionName, snapshot) {
        const rc = this.ref.collection(collectionName).doc(snapshot.ref.id);
        batch.create(rc, convertor_1.convertToInput({ createdAt: new Date(), updatedAt: new Date() }));
    }
    saveNestedCollection(collectionName, snapshot) {
        const rc = this.ref.collection(collectionName).doc(snapshot.ref.id);
        return rc.create(convertor_1.convertToInput(snapshot.data));
    }
    saveNestedCollectionWithBatch(batch, collectionName, snapshot) {
        const rc = this.ref.collection(collectionName).doc(snapshot.ref.id);
        batch.create(rc, convertor_1.convertToInput(snapshot.data));
    }
    ...
    update(data) {
        ...
        return this.ref.update(convertor_1.convertToInput(data));
    }
    updateWithBatch(batch, data) {
        ...
        batch.update(this.ref, convertor_1.convertToInput(data));
    }
    delete() {
